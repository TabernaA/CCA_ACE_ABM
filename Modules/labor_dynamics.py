...
def labor_search(unique_id, employer_ID, model,  region):
    if employer_ID == None:
        ...
        region_employers = model.datacollector.model_vars["Firms_regions"][int(model.schedule.time)][region]
        ...
        if len(suitable_employers_IDs) > 0:
            # due to bounded rationality, choose from a subset of all suitable employers, in this case is 1/4 of the total
            possible_employers_IDs = random.sample(suitable_employers_IDs, math.ceil(len(suitable_employers_IDs)/4))
...
def labor_search_cap(unique_id, employer_ID, model, region):
    if employer_ID == None:
        suitable_employers_IDs = []
        region_employers = model.datacollector.model_vars["Firms_regions"][int(model.schedule.time)][region]
        region_employers = region_employers[0]
        # find all firms with open vacancies in my region
        for i in range(len(region_employers)):
            if ( region_employers[i][0].open_vacancies == True):
                suitable_employers_IDs.append(region_employers[i][1])
        #print("SUITABLE EMPLOYERS IDS", suitable_employers_IDs)
...
def labor_search_cons(unique_id, employer_ID, model, region):
    if employer_ID == None:
        suitable_employers_IDs = []
        region_employers = model.datacollector.model_vars["Firms_regions"][int(model.schedule.time)][region]
        region_employers = region_employers[1]
        # find all firms with open vacancies in my region
        for i in range(len(region_employers)):
            if ( region_employers[i][0].open_vacancies == True):
                suitable_employers_IDs.append(region_employers[i][1])
        ...
        if len(suitable_employers_IDs) > 0:
            ...
            if len(possible_employers_IDs) > 0:
                ...
                employer = model.schedule.agents[possible_employers_IDs[best_i]]
                employer.employees_IDs.append(unique_id)
                #print("gained employee")
                #print("Employees list", employer.employees_IDs)
                employer_ID = possible_employers_IDs[best_i]
...
def labor_demand(capital_vintage, feasible_production):
    ...
    num_machines = len(capital_vintage)
    for i in range(1, num_machines+1):
        if (Q <= feasible_production):
            machines_used.append(capital_vintage[num_machines - i])
            Q += np.prod(capital_vintage[num_machines - i].amount)
   


    ## weighted average productivity of the machines I want to use ##
    average_productivity =  round((sum( [v.amount * v.productivity for v in machines_used] ) + 1) / (sum([v.amount for v in machines_used]) +1), 5)
