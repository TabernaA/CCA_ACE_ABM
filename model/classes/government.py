...
''' 
    ...
    def comp_norm(self):
        if self.region == 0: 
            ...
            COMP_all_0 = [ (1 + a.competitiveness[0])  for a in self.model.firms2 ]
            COMP_all_1 = [ (1 +  a.competitiveness[1]) for a in self.model.firms2 ]
            ids_all =  [a for a in self.model.ids_firms2] 
    ...
    def price_and_unfilled_demand_cons(self):
        if self.region == 0: 
            MS0 = [a.price for a in self.model.firms2 ] 
            #print(self.comp_normalized)rice for a in self.model.schedule.agents if ( a.type == "Cons")]
            MS1 = [a.unfilled_demand for a in self.model.firms2 ]
            ids = [a.unique_id for a in self.model.firms2 ]
    ...
    def entry_exit_variable(self, o = 0.50, x1=-0.15, x2=0.15):
        # exit firms
        if self.region == 0: 
            all_firms = [a for a in self.model.firms_1_2 ]
   ...
   ''' 
    ...
    def entry_exit(self):
        # exit firms
        all_firms_cons = [a for a in self.model.firms2 ]
        all_firms_cap = [a for a in self.model.firms2 ]
        all_firms = [a for a in self.model.firms_1_2]
        for firm in all_firms:
            if firm.lifecycle > 10:
                if  sum(firm.market_share) < 0.00001 or firm.net_worth < 0:
                # fire employees
                    ...
                    if firm.type == "Cap" and firm.net_worth  < 0:
                        firm.net_worth = average_regional_NW * 0.9
                        firm.wage = regional_wage    
                        prod0 = [a.productivity[0] for a in self.model.firms1 ]
                        prod1 = [a.productivity[1] for a in self.model.firms1 ]
                        #print(prod0)
                        firm.productivity[0] = max(prod0) * (1 - 0.15 + beta.rvs(3,3)*(0.15 + 0.15)) 
                        firm.productivity[1] = max(prod1) * (1 - 0.15 + beta.rvs(3,3)*(0.15 + 0.15)) 
                        firm.price = current_average_price
